...
% Q = 100 * diag([6, 6, 0.1, 0]);      % State cost matrix
...
for i = 1:floor(duration/timestep)
    ...
    if i == 1
        ...
        [x_star_mpc, u_star_mpc, dx_mpc] = create_mpc_star_input_changable_3u(x_star, u_star,...
    ...
    elseif mod(i*timestep, control_frequency) == 0 || i == 2
        ...
        u(:,i) = du(:,i) + u_star(:,i);
        du_extra_1 = mpc_output_use(4*(N+1)+4 : 4*(N+1)+6);
        u_extra_1 = du_extra_1 + u_star(:,i+mpc_timestep/timestep);
        du_extra_2 = mpc_output_use(4*(N+1)+7 : 4*(N+1)+9);
        u_extra_2 = du_extra_2 + u_star(:,i+2*mpc_timestep/timestep);
        [x_star_mpc, u_star_mpc, dx_mpc] = create_mpc_star_input_changable_3u(x_star, u_star,...
                                    N, i, timestep_parameter, control_frequency,...
                                    [u(:,i), u_extra_1, u_extra_2], x(:,i), len, radius, dp(:,i), timestep, ...
                                    L, mass, I_object);
        tic;
        ...
        fprintf('Error is: %2.4f %2.4f %2.4f %2.4f %2.4f\n', dx(1,i), dx(2,i), dx(3,i), dx(4,i), sqrt(dx(1,i)^2 + dx(2,i)^2));
        k = k + 1;
    elseif mod(i*timestep, control_frequency) == control_frequency/3
        du(:,i) = mpc_output_use(4*(N+1)+4 : 4*(N+1)+6);
        z(:,i) = mpc_output_use(4*(N+1)+3*N+4 : 4*(N+1)+3*N+6);
        u(:,i) = du(:,i) + u_star(:,i);
    elseif mod(i*timestep, control_frequency) == 2*control_frequency/3
        du(:,i) = mpc_output_use(4*(N+1)+7 : 4*(N+1)+9);
        z(:,i) = mpc_output_use(4*(N+1)+3*N+7 : 4*(N+1)+3*N+9);
        u(:,i) = du(:,i) + u_star(:,i);    
    else
        u(:,i) = u(:,i-1);
        z(:,i) = z(:,i-1);
