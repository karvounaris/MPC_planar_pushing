function [solver, args] = create_MPC_MPCC_controller( ...
    ...
    X_star = SX.sym('X_star', n_states, N+1);
    obstacles = SX.sym('Obs', 3, n_obstacles);

    args = struct;

    args.lbx(1:4:4*(N+1), 1) = -inf;        % lower bound for x
    args.ubx(1:4:4*(N+1), 1) = inf;         % upper bound for x
    args.lbx(2:4:4*(N+1), 1) = 0;        % lower bound for y
    ...
    for i = 1:N
        ...
        g = [g; vy_object_frame];
        args.lbg(constraint_offset,1) = 0;
        args.ubg(constraint_offset,1) = 0.06;
        constraint_offset = constraint_offset + 1;
    ...
    opts.knitro.maxit = 500;
    opts.knitro.xtol = 1e-8;
    ...
    opts.knitro.scale = 3;
    opts.knitro.maxtime = 0.5;
    % opts.knitro.honorbnds = 1;

    nlp_prob = struct('f', obj, 'x', opt_var, 'g', g, 'p', [X_0(:); X_star(:); obstacles(:)]);
