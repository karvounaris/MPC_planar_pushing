...
%     bineq = [];                       % Inequality constraint RHS
% 
%     Aeq_init = [eye(4), zeros(4, 4*(N/5+1) + 3*(N/5+1) + 3*(N/5+1))];
%     beq_init = dx_0;
%     Aeq = [Aeq; Aeq_init];
%     beq = [beq; beq_init];
% 
...
%     for i = 1:N
% 
%         % NOTE: the values below may change. Depends on the value of N
%         if i == 1
%             h = timestep;
%             j = j + 1;
%         elseif i + 4 == N + 1
...
% 
%         % Binary constraint z1 + z2 + z3 = 1
%         Aeq_binary = [zeros(1, 4*((N/5+1)+1) + 3*(N/5+1)), zeros(1, 3*(j-1)), 1, 1, 1, zeros(1, 3*((N/5+1)-j))];
%         beq_binary = 1;
%         Aeq = [Aeq; Aeq_binary];
%         beq = [beq; beq_binary];
% 
...
%         A_d = sys_d.A;
%         B_d = sys_d.B;
%         Aeq_dyn(1:4, 1:4*((N/5+1)+1)+3*(N/5+1)+3*(N/5+1)) = [zeros(4, (j-1)*4), A_d -eye(4), zeros(4, ((N/5+1)-j)*4), ... 
%                                              zeros(4, (j-1)*3), B_d, zeros(4, ((N/5+1)-j)*3), ...
%                                              zeros(4, 3*(N/5+1))];
%         % Dynamics constraints at each step
%         % Aeq_dyn(1:4, 1:4*(N+1)+3*N+3*N) = [zeros(4, (i-1)*4), eye(4) + h * A, -eye(4), zeros(4, (N-i)*4), ... 
%         %                                      zeros(4, (i-1)*3), h * B, zeros(4, (N-i)*3), ...
...
%         % Friction cone constraints
%         % Constraint 1: |f_t| <= mu * f_n
%         Aineq_friction_cone1 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), -mu, 1, 0, zeros(1, ((N/5+1)-j)*3), ...
%                                 zeros(1, 3*(N/5+1))];  % Ensure f_t <= mu * f_n
%         bineq_friction_cone1 = mu*u_star(1,i) - u_star(2,i);
%         Aineq_friction_cone2 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), -mu, -1, 0, zeros(1, ((N/5+1)-j)*3), ...
%                                 zeros(1, 3*(N/5+1))];  % Ensure f_t >= -mu * f_n
%         bineq_friction_cone2 = mu*u_star(1,i) + u_star(2,i);
% 
%         % Constraint 2: f_n >= 0
%         Aineq_friction_cone3 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), -1, 0, 0, zeros(1, ((N/5+1)-j)*3), ...
%                                 zeros(1, 3*(N/5+1))];  % Ensure f_n >= 0
%         bineq_friction_cone3 = u_star(1,i);
% 
%         % Mode constraints (Big-M constraints for mode switching)
%         % Sticking Mode (z1 = 1): phi_dot = 0
%         Aineq_stick_1 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), 0, 0, 1, zeros(1, ((N/5+1)-j)*3), ...
%                          zeros(1, 3*(j-1)), M, 0, 0, zeros(1, 3*((N/5+1)-j))];
%         bineq_stick_1 = - u_star(3,i) + M;
%         Aineq_stick_2 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), 0, 0, -1, zeros(1, ((N/5+1)-j)*3), ...
%                          zeros(1, 3*(j-1)), M, 0, 0, zeros(1, 3*((N/5+1)-j))];
%         bineq_stick_2 = u_star(3,i) + M;
% 
%         % Sliding Left Mode (z2 = 1): phi_dot < 0 and f_t = mu * f_n
%         Aineq_slide_left1 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), -mu, 1, 0, zeros(1, ((N/5+1)-j)*3), ...
%                              zeros(1, 3*(j-1)), 0, M, 0, zeros(1, 3*((N/5+1)-j))];
%         bineq_slide_left1 = mu*u_star(1,i) - u_star(2,i) + M;
%         Aineq_slide_left2 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), mu, -1, 0, zeros(1, ((N/5+1)-j)*3), ...
%                              zeros(1, 3*(j-1)), 0, M, 0, zeros(1, 3*((N/5+1)-j))];
%         bineq_slide_left2 = -mu*u_star(1,i) + u_star(2,i) + M;
%         Aineq_slide_left3 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), 0, 0, 1, zeros(1, ((N/5+1)-j)*3), ...
%                              zeros(1, 3*(j-1)), 0, M, 0, zeros(1, 3*((N/5+1)-j))];
%         bineq_slide_left3 = -u_star(3,i) - eps + M;
% 
%         % Sliding Right Mode (z3 = 1): phi_dot > 0 and f_t = -mu * f_n
%         Aineq_slide_right1 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), mu, 1, 0, zeros(1, ((N/5+1)-j)*3), ...
%                              zeros(1, 3*(j-1)), 0, 0, M, zeros(1, 3*((N/5+1)-j))];
%         bineq_slide_right1 = -mu*u_star(1,i) - u_star(2,i) + M;
%         Aineq_slide_right2 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), -mu, -1, 0, zeros(1, ((N/5+1)-j)*3), ...
%                              zeros(1, 3*(j-1)), 0, 0, M, zeros(1, 3*((N/5+1)-j))];
%         bineq_slide_right2 = mu*u_star(1,i) + u_star(2,i) + M;
%         Aineq_slide_right3 = [zeros(1,4*((N/5+1)+1)), zeros(1, (j-1)*3), 0, 0, -1, zeros(1, ((N/5+1)-j)*3), ...
%                              zeros(1, 3*(j-1)), 0, 0, M, zeros(1, 3*((N/5+1)-j))];
%         bineq_slide_right3 = u_star(3,i) - eps + M;
% 
%         % Add mode constraints to inequality constraints
...
% 
%     % Initialize block diagonal components for model.Q
%     Q_blocks = repmat({Q}, 1, (N/5+1)-1);  % Q for each time step except the terminal state
%     R_blocks = repmat({R}, 1, (N/5+1));    % R for each time step for control input costs
...
%     % model.ub = [inf * ones(4 * (N+1), 1); 100 * ones(3 * N, 1); ones(3 * N, 1)];
%     % Initialize bounds for state variables
%     model.lb = -inf * ones(4 * ((N/5+1)+1), 1);
%     model.ub =  inf * ones(4 * ((N/5+1)+1), 1);
% 
%     % Define bounds for inputs (u = [fn, ft, phi_dot])
%     lb_u = [-100; -100; -10];
%     ub_u = [ 100;  100;  10];
% 
%     % Repeat input bounds for N steps
%     model.lb = [model.lb; repmat(lb_u, (N/5+1), 1)];
%     model.ub = [model.ub; repmat(ub_u, (N/5+1), 1)];
% 
%     % Add binary variable bounds
%     model.lb = [model.lb; zeros(3 * (N/5+1), 1)];
%     model.ub = [model.ub; ones(3 * (N/5+1), 1)];
%     model.vtype = [repmat('C', 1, 4 * ((N/5+1)+1) + 3 * (N/5+1)), repmat('B', 1, 3 * (N/5+1))];
% 
%     % Set the remaining fields in model (A, rhs, Q, etc.) as in your code
%     model.A = sparse([Aeq; Aineq]);
%     model.rhs = [beq; bineq];
