...
x_star_extension = repmat(x_star(:, end), 1, delayms/timestep + N*timestep_parameter); % Repeat last column of x_star N times
u_star_extension = zeros(size(u_star, 1), delayms/timestep + N*timestep_parameter);    % Create zero matrix for u_star
...
for i = 1:floor(duration/timestep)
    ...
    % elseif abs(solver_times(end) - (time(i) - mpc_timestamps(k-1))) <= 1e-5 || i == 2
    elseif mod(i*timestep, control_frequency) == 0
        % if k > 6
        %     temp = solver_times(end) * 1e3;
        %     for l = 1:k-2
        %     temp = temp + solver_times(end-l) * 1e3;
        %     end
        %     delayms = round(temp/(k-1)) / 1e3;
        %     delayms_list(k) = delayms;
        % end
        [x_star_mpc, u_star_mpc, dx_mpc] = create_mpc_star_input(x_star, u_star,...
                                    N, i, timestep_parameter, delayms,...
