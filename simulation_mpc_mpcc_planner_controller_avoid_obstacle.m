...
x(:,1) = [0, 0, -pi/2, 3*pi/2];
x_end = [1; 0; -pi/2; 3*pi/2];
% obstacle_data = [[0.1; 0.2; 0.05], [0.42; 0.08; 0.05], [0.4; 0.42; 0.06]];  % [x_obstacle, y_obstacle, radius_obstacle]
obstacle_data = [0.4; 0.42; 0.05];  % [x_obstacle, y_obstacle, radius_obstacle]
% obstacle_data = [];
...
% Q = 20 * diag([4, 4, 0.0001, 0]);
Q = 0 * diag([4, 4, 0.0001, 0]);
QN = 10000 * diag([5, 5, 0.005, 0]);
...
[solver_controller, args] = create_MPC_MPCC_planner_controller_avoid_obstacle( ...
...
while sqrt(dx_loop(1)^2 + dx_loop(2)^2)>0.01 || abs(dx_loop(3)) > 0.1
    ...
    if i == 1
        ...
        moving_obs_x = 0.5 + 0 * sin(2*pi*0*time(i));
        moving_obs_y = 0.1 + 0.1 * sin(2*pi*0.02*time(i));
        obstacle_data = [moving_obs_x; moving_obs_y; 0.05];
        obstacle_traj(:, i) = [moving_obs_x; moving_obs_y];
        args.p = [x(:,i); x_end; obstacle_data(:)];
        tic;
        ...
        fprintf('Time is: %g\n', time(i));
        fprintf('Error is: %2.4f %2.4f %2.4f %2.4f %2.4f\n', dx(1,i), dx(2,i), dx(3,i), sqrt(dx(1,i)^2 + dx(2,i)^2), sqrt(dx(1,i)^2 + dx(2,i)^2 + dx(3,i)^2));
    ...
    elseif (i * timestep - last_time_MIQP_start > solver_times(end) || i == 2)
        ...
        moving_obs_x = 0.5 + 0 * sin(2*pi*0*time(i));
        moving_obs_y = 0.1 + 0.1 * sin(2*pi*0.08*time(i));
        obstacle_data = [moving_obs_x; moving_obs_y; 0.05];
        obstacle_traj(:, i) = [moving_obs_x; moving_obs_y];
        u(:,i) = u(:,i-1);
        eps(:,i) = eps(i-1);
...
videoFilename = 'simulation.avi';
video = VideoWriter(videoFilename);
video.FrameRate = 15;  % Adjust as needed
open(video);
...
for i = 1:20:length(x(1,:))
    ...
    % --- (B) Plot the capsule (object) shape at the current step
    if object_shape == "rectangular_capsule_prism"
    ...
    % --- (C) Plot the contact point trajectory up to current index
    plot(contact_x_world(1:i), contact_y_world(1:i), 'r-', 'LineWidth', 2, ...
         'DisplayName', 'Contact Path');

    % --- (D) Plot ALL obstacles at each frame
    for j = 1:size(obstacle_data, 2)
        if j == 1 && i <= size(obstacle_traj, 2)
            obs_x = obstacle_traj(1, i);
            obs_y = obstacle_traj(2, i);
        else
            obs_x = obstacle_data(1, j);
            obs_y = obstacle_data(2, j);
        end
        ...
        if j == 1
            % Add DisplayName for first obstacle only
            fill(obs_shape(:,1), obs_shape(:,2), 'k', ...
                 'FaceAlpha', 0.3, 'EdgeColor', 'k', 'LineWidth', 1.5, ...
                 'DisplayName', 'Obstacle');
        else
            fill(obs_shape(:,1), obs_shape(:,2), 'k', ...
    ...
    xlabel('x (m)');
    ylabel('y (m)');
    % legend('Location','Best');  % optional

    % --- (F) Capture frame and write to video
    drawnow;
    frame = getframe(fig);
    writeVideo(video, frame);
end

% --- 3) Close video writer
close(video);
