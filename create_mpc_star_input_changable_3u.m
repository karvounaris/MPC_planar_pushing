function [x_star_mpc, u_star_mpc, dx_mpc] = create_mpc_star_input_changable_3u(x_star, u_star, ...
    ...
    if iteration ~= 1
        for i = 1:control_frequency/timestep
            if mod(i*timestep, control_frequency) < (control_frequency/3)+1
                u_simulated = u(:,1);
            elseif mod(i*timestep, control_frequency) < (2*control_frequency/3)+1
                u_simulated = u(:,2);
            else 
                u_simulated = u(:,3);
            end
